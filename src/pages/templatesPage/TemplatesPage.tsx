...
export default function TemplatesPage() {
  ...
  const groups = React.useMemo(() => {
    ...
    pages.forEach((page) => {
      const section = page.section || "Autres";
  ...
  const filtered = React.useMemo(
    () => pages.map((page, index) => ({ page, index })).filter(({ page }) => !query || normalize(`${page.title} ${page.content}`).includes(normalize(query))),
    [pages, query]
  );
    ...
    <div className="kb-page">
      <aside className="kb-sidebar">
        ...
        <div className="kb-nav">
          {query
            ? filtered.map(({ page, index }) => <button key={index} className={index === selected ? "kb-item active" : "kb-item"} onClick={() => setSelected(index)}>{page.title} <small>p. {page.page}</small></button>)
            : groups.map(([name, group]) => <div className="kb-group" key={name}>
      ...
      <main className="kb-content">
        {error ? <div className="kb-error">Impossible de charger la KB : {error}</div> : current ? <>
          ...
          <h1>{current.title}</h1>
          <div className="kb-body">{current.content.split(/\n{2,}/).map((part, index) => <p key={index}>{part}</p>)}</div>
