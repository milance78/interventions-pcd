...
const removeAddressClientsBlock = (value: string) =>
  value
    .replace(
      /(?:^|\n\n?)(?:Un TF à l'adresse:|Un TF à l'adresse,|Un client TF à l'adresse:|Client à l'adresse:|L'UTAC à l'adresse occupé par:)[^\n]*/gi,
      "",
    )
    .replace(
      /(?:^|\n\n?)(?:Clients TF à l'adresse:|Les UTAC à l'adresse occupés par:|Les clients TF à l'adresse:|Clients à l'adresse:)\n(?:\d+\.\s*[^\n]*\n?)*/gi,
      "",
    )
    .replace(/^\n+|\n+$/g, "")
    .replace(/\n{3,}/g, "\n\n");
